...
- Bản demo không triển khai duyệt lại khi sửa phiếu. Nếu sau này client yêu cầu, sẽ bổ sung trạng thái `REVISION_PENDING` và quyền `inventory.document.approve_revision`.

## Phương án được chọn

Chọn phương án: **thêm lớp phiếu nhiều dòng phía trên sổ cái hiện tại**.

Không chọn vá tiếp các form hiện tại vì nhập/xuất/chuyển đang là giao dịch một dòng, sẽ khó xử lý sửa phiếu, nhiều dòng, chuyển kho và báo cáo định mức.

Không chọn viết lại toàn bộ một lần vì phạm vi quá rộng. Cần chuyển lõi theo từng giai đoạn để giữ hệ thống chạy được.

## Phạm vi tạm hoãn cho bản demo

Chức năng phê duyệt chuyển kho tạm thời không triển khai.

Quy tắc bản demo:

- Phiếu chuyển kho sau khi ghi sổ sẽ phát sinh tồn kho ngay.
- Không có trạng thái chờ kho nhận duyệt.
- Không cần thủ kho nhận xác nhận.
- Sau khi hệ thống ổn định, có thể bổ sung lại luồng duyệt chuyển kho bằng trạng thái `PENDING_APPROVAL` và quyền `inventory.transfer.approve`.
...
- `createdById`, `updatedById`, `postedById`, `voidedById`
...
- Ngày chứng từ.
- Loại phiếu.
- Trạng thái bằng màu dễ phân biệt: nháp, đã ghi sổ, đã hủy.
- Người lập phiếu.
...
3. Lưu nháp hoặc ghi sổ.
4. Khi ghi sổ, hệ thống kiểm tra tồn từng vật tư tại kho nguồn.
5. Nếu đủ tồn, hệ thống sinh cặp movement ngay:
   - `OUT / TRANSFER_OUT` ở kho nguồn.
   - `IN / TRANSFER_IN` ở kho nhận.

Luồng duyệt kho nhận được hoãn sau giai đoạn demo.
...
- Ghi sổ ngay khi người dùng lưu phiếu chính thức.
...
### Giai đoạn 3: Chuyển kho ghi sổ ngay

- Phiếu chuyển nhiều dòng.
- Kiểm tra tồn kho nguồn trước khi ghi sổ.
- Sinh `TRANSFER_OUT` và `TRANSFER_IN` trong cùng transaction.
- Hoãn luồng phê duyệt kho nhận sang giai đoạn sau demo.
...
- Chuyển kho ghi sổ ngay và làm giảm tồn kho nguồn, tăng tồn kho nhận trong cùng transaction.
